...
// then pushed the response past max_bytes by a two-digit sliver.
{
  ...
  mkAgent(s, "b");
  s.joinRoom(probe, "b", {});
  s.postMessage(probe, "b", "x".repeat(100), "text", null, null, null);
  ...
  for (let i = 0; i < 55; i++) {
    s.postMessage(r, "b", "x".repeat(perRow - rowEnv), "text", null, null, null);
  }
...
// --- my_mentions dense inbox at max_bytes=400000 ------------------------------
{
  ...
  s.joinRoom(r, "b", {});
  for (let i = 0; i < 110; i++) {
    s.postMessage(r, "b", "y".repeat(3900), "text", ["a"], null, null);
  }
  const MAX = 400_000;
  const seen = new Set();
  let after = 0;
...
// --- search_messages dense matches stay within the default budget -------------
{
  ...
  s.joinRoom(r, "b", {});
  for (let i = 0; i < 60; i++) {
    s.postMessage(r, "b", "needle " + "z".repeat(1990), "text", null, null, null);
  }
...
// room name / sender type / role let stubs escape a 1000-char budget.
{
  ...
  s.joinRoom(room, "a", {});
  s.joinRoom(room, sender, {});
  s.postMessage(room, sender, "\u0001".repeat(400), "text", ["a"], null, null);
...
// --- get_thread shares the hard bound ------------------------------------------
{
  ...
  s.postMessage(r, "a", "p".repeat(1800), "text", null, null, null); // seq 1
  s.postMessage(r, "b", "f".repeat(99_700), "text", null, 1, null); // seq 2
  ...
  s.postMessage(r, "b", "\u0001".repeat(200_000), "text", mentions, null, null); // seq 3
  ...
  s.postMessage(r, "a", "root", "text", null, null, null); // seq 4
  for (let i = 0; i < 60; i++) {
    s.postMessage(r, "b", "w".repeat(1990), "text", null, 4, null);
  }
